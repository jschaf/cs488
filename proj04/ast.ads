...
package AST is
   ...
   -- A symbol table entry is just an id and corresponding value.
   ...
   -- Since the word "type" is an Ada keywored, we'll use the word "Tag"
   ...
   -- id references to definitions that require parameters.  It does
   -- type checking and other semantic analysis at the same time.  The
   -- input tree is not changed.  A completely fresh tree is returned
   -- the expanded tree.  The given symbol table holds name-to-definition
