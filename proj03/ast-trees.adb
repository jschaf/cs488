...
with Ada.Text_IO, Ada.Characters.Handling, Ada.Containers;
use  Ada.Text_IO, Ada.Characters.Handling, Ada.Containers;
...
package body AST.Trees is
   ...
   begin
      ...
      Unbox(Tree.Sensor);
      Find_Value_With_Default(Tree.Vulnerability, 1.0, Tree.Hard_Vulnerability);
   end Finish;
   ...
   begin
      ...
      Find_Value_With_Default(Tree.Effectiveness, 1.0, Tree.Hard_Effectiveness);
      Find_Value_With_Default(Tree.Vulnerability, 1.0, Tree.Hard_Vulnerability);
   end Finish;
   ...
   procedure Open(Tree : access Node_Type'Class; XML_Tag : in String) is
      Tag_Image : String := To_Lower(Tag_Type'Image(Tree.Tag));
   begin
      Put("<" & XML_Tag & " tag=""" &
          Tag_Image(Tag_Image'First .. Tag_Image'Last - 4) &
          """>");
   end Open;
   ...
   procedure Find_Threat_Effectiveness (Threat : in Threat_Ptr_Type; 
                                        Value : out Real) is
   begin
      Value := Threat.Hard_Effectiveness;
      --  Find_Positive_Value(Threat.Effectiveness, Value);
